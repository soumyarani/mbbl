...
class Encoder():
    ...
    def __init__(self, state_dim, latent_dim, action_dim):
        ...
        self.encoder = MLPnetwork1(self.state_dim, self.latent_dim, dim_hidden=64).to(device)
        self.forward_dynamics = MLPnetwork2(self.latent_dim, self.action_dim, self.latent_dim, dim_hidden=64).to(device)
        self.inverse_dynamics = MLPnetwork2(self.latent_dim, self.latent_dim, self.action_dim, dim_hidden=64).to(device)

        self.optimizer_encoder = optim.Adam(self.encoder.parameters(), lr=0.001)
        self.optimizer_fdyn = optim.Adam(self.forward_dynamics.parameters(), lr=0.001)
        self.optimizer_idyn = optim.Adam(self.inverse_dynamics.parameters(), lr=0.001)
